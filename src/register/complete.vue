      ...
      <div class="complete-content" ref="completeContent">
        <div>
          <div class="complete-card">
            ...
            <div class="info-item">
              <div class="info-item__value">
                <popup-picker title="类别" :data="cateList" v-model="category" class="transCate" @on-hide="changeCate" placeholder="选择类别" @on-show="showCategory()"></popup-picker>
              </div>
            </div>
            <div class="info-item">
              <div class="info-item__value">
  ...
  export default {
    ...
    data () {
      return {
        ...
        checkLincense: false,
        completeScroll: null,
        newGKList: ['110000', '310000', '330000'],
        selectSubject: '',
        location: '正在定位',
        latitude: '', // 纬度
        longitude: '', // 经度
    ...
    //   value (newVal, oldVal) {
    //     // immediate: true,
    //     console.log('value值得变化：', newVal, oldVal, newVal === oldVal)
    //     if (!(newVal.toString() === oldVal.toString())) {
    //       this.$vux.toast.text('地区发生了变化，请重新选择类别和批次呦~')
    //     }
    //   },
    //   category (newVal, oldVal) {
    //     if (!(newVal.toString() === oldVal.toString())) {
    //       this.$vux.toast.text('类别发生了变化，请重新选择批次呦~')
    //     }
    //   }
    // },
    mounted () {
      this.init()
      // this.addClick()
      // console.log(this.$store.state.user.locationInfo)
    ...
    methods: {
      ...
      addClick () {
        const _this = this
        document.querySelector('.transCate').addEventListener('click', function () {
          if (_this.value.length === 0) {
            errorMsg(_this, '请先选择地区')
            return false
          }
      ...
      getAllCategory () {
        getAllCategory(this.provinceCode).then(res => {
      ...
      logHide (str) {
        // console.log('隐藏变化：', this.newVal, this.value, this.newVal.toString() === this.value.toString())
        // console.log('隐藏变化2：', this.newVal.length, this.value.length, this.newVal.toString() === this.value.toString())
        // console.log('on-hide', str)
        if (this.newVal.length > 0 && this.newVal.toString() !== this.value.toString()) {
          this.$vux.toast.text('地区发生了变化，请重新选择类别和批次呦~')
          this.category = []
          this.batch = []
        }
      ...
      onShadowChange (ids, names) {
        ...
        this.city = names[1]
        this.address = names[2]
        this.provinceCode = ids[0]
        this.addressCode = ids[2]
        this.cityCode = ids[1]
        if (this.newGKList.indexOf(ids[0]) > -1) {
          this.showNewGK = true
        } else {
          this.showNewGK = false
        }
        console.log('位置；', this.newVal, this.value)
      },
      // changeProv (value) {
      ...
      // },
      changeCate (val) {
        // console.log('类别变化：', this.newCate, this.category)
        // console.log('类别变化2：', this.newCate[0].length, this.category[0].length)
        if (this.newCate[0].length > 0 && this.newCate.toString() !== this.category.toString()) {
          this.$vux.toast.text('类别发生了变化，请重新选择批次呦~')
          this.batch = []
        }
      },
      ...
      showCategory () {
        this.newCate = this.category
        // console.log('打开类别：', this.value)
        if (this.value.length === 0) {
          errorMsg(this, '请先选择地区')
          return
        }
        this.getAllCategory()
      },
      showBatch () {
        console.log('类别：', this.category === '')
        console.log('类别：', this.category)
        if (this.value.length === 0) {
          errorMsg(this, '请先选择地区')
          return
        ...
        getAllBatch(this.provinceCode, this.category[0]).then(res => {
      ...
      submit () {
        ...
        const prov = this.provinceCode
        // const prov = this.newProvince
      ...
      getUserInfo () {
        myInfo(this.openid).then(res => {
          ...
          if (res.data.code === 0) {
            ...
            this.value.push(res.data.data.provincecode)
            this.value.push(res.data.data.citycode)
            this.value.push(res.data.data.addresscode)
