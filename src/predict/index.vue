...
<!--          <input type="text" class="predict_score" v-model="score" maxlength="3" @blur="gotoBlur" @input="handleInput" @click="toggleInput" ref="scoreInput" placeholder="点击请输入成绩">-->
      ...
      <div class="predict_four">
        <span class="predict_four_button" @click="startSchoolPredict">智能预测</span>
...
<!--      <div class="predict__pay predict__list&#45;&#45;noview" v-if="!loading && noview && list.length !== 0" @click="toPay">-->
      <div class="predict__pay_wen predict__list--wennoview" v-if="!loading && noview && list.length !== 0 && curTab === 'wen'" @click="toPay">
<!--      <div class="predict__pay predict__list&#45;&#45;noview" v-if="!loading && noview && list.length !== 0 && curTab === 'tiao'" @click="toPay">-->
        <div class="pay_tip">
          付费提示: 稳妥学校学校推荐列表为付费项，每改变一次预测分数和批次需额外收费，付费解锁后会推荐15所高校以供参考。
        </div>
      ...
      <div ref="schoolWrapper" class="predict__list">
          <div>
            ...
            <div class="school-list" :class="{'noview': noview}" v-if="curTab !== 'wen'">
              ...
              <div class="school_item_info" v-if="!loading && list.length !== 0" v-for="(school,index) in list" @click="gotoDetailPage(school)" :key="index">
<!--              <div class="school-item" v-if="!loading && list.length !== 0" v-for="(school,index) in list" @click="gotoDetailPage(school)" :key="index">-->
                <div class="school-item">
                  <div class="school-item__logo"><img :src="school.logo"></div>
                  <div class="add_volunteer" @click.stop="addVolunteer(school.schoolname)"><i class="iconfont iconiconjia"></i></div>
                ...
                <div class="predict__pay predict__list--noview" v-if="!loading && noview && list.length !== 0 && curTab !== 'wen' && index === 3" @click="toPay">
                  <!--      <div class="predict__pay predict__list&#45;&#45;noview" v-if="!loading && noview && list.length !== 0 && curTab === 'tiao'" @click="toPay">-->
                  <div class="predict__paytext">
                    <i class="iconfont iconsuo"></i>
                    <span class="pay-tips">解锁全部学校</span>
                  ...
                  <div class="pay_tip">
                    付费提示: 学校推荐列表为付费项，每改变一次预测分数和批次需额外收费，付费解锁后会推荐15所高校以供参考。
                  </div>
                </div>
              </div>
            ...
            <div class="school-list" :class="{'noview_wen': noview}" v-if="curTab === 'wen'" @click="toPay">
<!--            <div class="school-list" :class="{'noview_wen': noview}" v-if="curTab === 'wen'" @click="toPay">-->
              <div class="list-loading" v-if="loading">
                <div class="loader"></div>
              </div>
              ...
              <div class="school-item" v-if="!loading && list.length !== 0" v-for="school in list" @click.stop="gotoDetailPage(school)">
                <div class="school-item__logo"><img :src="school.logo"></div>
                <div class="add_volunteer" @click.stop="addVolunteer(school.schoolname)"><i class="iconfont iconiconjia"></i></div>
                <div class="school-item__devide"></div>
                <div class="school-item__info">
            ...
            <!--        </div>-->
<!--            <div class="view_volunteer" v-if="!loading && list.length !== 0">-->
<!--              <div class="view_info"><i class="iconfont iconchakan3"></i></div>-->
<!--            </div>-->
          </div>
      </div>
      <div class="view_volunteer" v-if="!loading && list.length !== 0" @click="viewVolunteer">
        <div class="view_info"><i class="iconfont iconmubiao"></i></div>
<!--        <div class="view_info"><i class="iconfont iconchakan3"></i></div>-->
      </div>
    </div>
    ...
    <div class="predict__tips" v-show="showVolunteer">
      <div class="predict__tips__overlay" @click="noViewVolunteer"></div>
      <div class="predict__tips__content_volunteer">
        <div class="tips__header">志愿清单</div>
        <div class="tips_summary">当前批次是{{batch}}，分数为{{score}}分，所在地是{{province}}。</div>
        <div class="tips__body">
...
<!--        </div>-->
    ...
    <toast v-model="showEnough" type="cancel">志愿已满</toast>
  </div>
</template>
<script>
  import { getYuce, getSchoolInfo, myInfo, getPaiming, getGoalCity, viewVolunteer, addVolunteer, delVolunteer } from '@/api/index'
  ...
  export default {
    ...
    mounted () {
      ...
      // console.log(localStorage.agreeDiag)
      // this.agreeDiag = 'show'
    ...
    methods: {
      ...
      addVolunteer (schoolName) { // 添加志愿单
        console.log('添加志愿', schoolName)
        addVolunteer({
          openid: this.openid,
          ...
          } else if (res.data.msg === '预约冲突' && res.data.code === 503) {
            this.$vux.toast.show({
              text: '已存在志愿单中~',
              width: '9em'
            })
          } else if (res.data.msg === '志愿已满' && res.data.code === 508) {
            this.showEnough = true
            // this.$vux.toast.show({
            //   text: '志愿单已满~',
            //   width: '9em'
            // })
...
<style lang="scss" scoped>
  ...
  .view_volunteer {
    ...
    .view_info {
      ...
      height: 45px;
      width: 45px;
      background-color: rgba(255, 158, 0, .9);
  ...
  .school-item {
    ...
    /*.noview & + & {*/
    /*  &:not(first-child):not(:nth-child(2)):not(:nth-child(3)){filter: blur(16px);}*/
    /*  border-top: 1px solid #f9f9f9;*/
    /*}*/
    /*.noview & {*/
  ...
  .noview .school_item_info + .school_item_info:not(first-child):not(:nth-child(2)):not(:nth-child(3)) .school-item{
    /*-webkit-filter: blur(16px);*/
    filter: blur(16px);
  }
  /*.noview .school_item_info:not(first-child) .school-item {*/
  /*  filter: blur(16px);*/
  /*}*/
  /*.school-item__logo {*/
  /*  position: absolute;*/
  ...
  .add_volunteer {
    ...
    width: 25px;
    color: #fff;
    background-color: rgba(33, 117, 224, .7);
    border-radius: 5px;
